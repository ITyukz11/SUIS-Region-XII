...
export default function Northcotabatotable(props) {
    ...
    const sortedData = (column) => {
      ...
      if (props.northCotData) {
        if (noNumberColumnDatas.includes(column)) {
          if (sortActive) {
            return props.northCotData.sort((a, b) => {
        ...
        console.log("no data fetch props.northCotData")
      }
    
      return props.northCotData;
    };
    

    // Get the current page's northCotData
    const currentData = props.northCotData? sortActive?
    sortedData().slice(startIndex,endIndex)
    :props.northCotData.slice(startIndex, endIndex):'';
  
    // Function to navigate to the next page
    const nextPage = () => {
      if (currentPage < Math.ceil(props.northCotData.length / itemsPerPage)) {
    ...
    const endPage = () => {
      const totalPages = Math.ceil(props.northCotData.length / itemsPerPage);
    ...
    // const handlesSortData = props.northCotData.sort((a, b) => {
    ...
    return (
      <div className='z-0'>
        ...
        <div className={`mt-4 flex justify-center gap-5 ${props.isMobile?'flex-wrap':''}`}>
          ...
          <label>
            {props.northCotData?props.northCotData.length:''} 
          </label>
          <button
            onClick={nextPage}
            disabled={currentPage === Math.ceil(props.northCotData.length / itemsPerPage)}
            className="bg-navy-primary text-white px-4 py-2 rounded-md disabled:bg-gray-300 hover:bg-opacity-95"
          >
            Next
          </button>
