...
export default function Southcotabatotable(props) {
    ...
    const sortedData = (column) => {
      ...
      if (props.southCotData) {
        if (noNumberColumnDatas.includes(column)) {
          if (sortActive) {
            return props.southCotData.sort((a, b) => {
        ...
        console.log("no data fetch props.southCotData")
      }
    
      return props.southCotData;
    };
    

    // Get the current page's southCotData
    const currentData = props.southCotData? sortActive?
    sortedData().slice(startIndex,endIndex)
    :props.southCotData.slice(startIndex, endIndex):'';
  
    // Function to navigate to the next page
    const nextPage = () => {
      if (currentPage < Math.ceil(props.southCotData.length / itemsPerPage)) {
    ...
    const endPage = () => {
      const totalPages = Math.ceil(props.southCotData.length / itemsPerPage);
    ...
    // const handlesSortData = props.southCotData.sort((a, b) => {
    ...
    return (
      <div className='z-0'>
        ...
        <div className={`mt-4 flex justify-center gap-5 ${props.isMobile?'flex-wrap':''}`}>
          ...
          <label>
            {props.southCotData?props.southCotData.length:''} 
          </label>
          <button
            onClick={nextPage}
            disabled={currentPage === Math.ceil(props.southCotData.length / itemsPerPage)}
            className="bg-navy-primary text-white px-4 py-2 rounded-md disabled:bg-gray-300 hover:bg-opacity-95"
          >
            Next
          </button>
