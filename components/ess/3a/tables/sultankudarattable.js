...
export default function Sultankudarattable(props) {
    ...
    const sortedData = (column) => {
      ...
      if (props.sultanKudaratData) {
        if (noNumberColumnDatas.includes(column)) {
          if (sortActive) {
            return props.sultanKudaratData.sort((a, b) => {
          ...
          props.sultanKudaratData.sort((a, b) => {
        ...
        console.log("no data fetch props.sultanKudaratData")
      }
    
      return props.sultanKudaratData;
    };
    

    // Get the current page's sultanKudaratData
    const currentData = props.sultanKudaratData? sortActive?
    sortedData().slice(startIndex,endIndex)
    :props.sultanKudaratData.slice(startIndex, endIndex):'';
  
    // Function to navigate to the next page
    const nextPage = () => {
      if (currentPage < Math.ceil(props.sultanKudaratData.length / itemsPerPage)) {
    ...
    const endPage = () => {
      const totalPages = Math.ceil(props.sultanKudaratData.length / itemsPerPage);
    ...
    // const handlesSortData = props.sultanKudaratData.sort((a, b) => {
    ...
    return (
      <div className='z-0'>
        ...
        <div className={`mt-4 flex justify-center gap-5 ${props.isMobile?'flex-wrap':''}`}>
          ...
          <label>
            {props.sultanKudaratData?props.sultanKudaratData.length:''} 
          </label>
          <button
            onClick={nextPage}
            disabled={currentPage === Math.ceil(props.sultanKudaratData.length / itemsPerPage)}
            className="bg-navy-primary text-white px-4 py-2 rounded-md disabled:bg-gray-300 hover:bg-opacity-95"
          >
            Next
          </button>
