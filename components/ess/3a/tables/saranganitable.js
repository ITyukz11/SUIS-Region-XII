...
export default function Saranganitable(props) {
    ...
    const sortedData = (column) => {
      ...
      if (props.saranganiData) {
        if (noNumberColumnDatas.includes(column)) {
          if (sortActive) {
            return props.saranganiData.sort((a, b) => {
        ...
        console.log("no data fetch props.saranganiData")
      }
    
      return props.saranganiData;
    };
    

    // Get the current page's saranganiData
    const currentData = props.saranganiData? sortActive?
    sortedData().slice(startIndex,endIndex)
    :props.saranganiData.slice(startIndex, endIndex):'';
    ...
    const endPage = () => {
      const totalPages = Math.ceil(props.saranganiData.length / itemsPerPage);
    ...
    // const handlesSortData = props.saranganiData.sort((a, b) => {
    ...
    return (
      <div className='z-0'>
        ...
        <div className={`mt-4 flex justify-center gap-5 ${props.isMobile?'flex-wrap':''}`}>
          ...
          <button
            onClick={nextPage}
            disabled={currentPage === Math.ceil(props.saranganiData.length / itemsPerPage)}
            className="bg-navy-primary text-white px-4 py-2 rounded-md disabled:bg-gray-300 hover:bg-opacity-95"
          >
            Next
          </button>
